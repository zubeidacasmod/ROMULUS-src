	...
	<article class="post hentry">
		...
		<header class="entry-header">
			<h1 class="entry-title">
				<a href="#" title="Post Heading" rel="bookmark">Metadata: BFO-QL</a>
			</h1>
...
$query = "SELECT `OntologyID`, `OntologyName`, `OntologyAcronym`, `OntologyDescription`,
 `CreationDate`, `LatestModifiedDate`, `Version`, `URI` FROM `Ontology` WHERE `OntologyID` = 66";
...
$query2 = "SELECT `LanguageName`, `LanguageAcronym`, `LanguageDescription` FROM `Language` WHERE `LanguageName` = (SELECT `LanguageName` from `Ontology` where `OntologyID` = 66)";
...
$query3 = "SELECT `DLExpressivity`, `NumberOfClasses`, `NumberOfIndividuals`, `NumberOfObjectProperties`,`NumberOfDataProperties`, `NumberOfAxioms`
 FROM `Metrics` WHERE `MetricsID` = (SELECT `MetricsID` from `Ontology` where `OntologyID` = 66)";
...
$query55 = "SELECT `LicenceName`, `LicenceDescription`, `LicenceHomePage` from `Licence` where `LicenceName`
 = (SELECT `LicenceName` from `Ontology` where `OntologyID` = 66)";
...
`PersonEmail` IN (SELECT `PersonEmail` from `OntologyPerson` where `OntologyID` = 66)";
...
`ProjectUsageApplication` from `Project` where `ProjectID` IN (SELECT `ProjectID` from `OntologyProject` where `OntologyID` = 66)";
...
`ModuleCompleteness`,`ModuleClassSize`, `ModulePropertySize`, `MethodType` from `Module` where `ModuleID` = 66";
...
(SELECT `OriginalOntologyID`  from `Module` where `ModuleID` = 66)";
...
where `ToolName` = (SELECT `ToolName` from `Module` where `ModuleID` = 66)";
...
?>
		...
		<footer class="entry-meta">
			Posted on <time class="entry-date" datetime="2066-05-01" pubdate>May 01 2066</time>
			by <span class="author vcard"><a class="url fn n" href="#" title="View all posts by Author" rel="author">Zubeida Casmod Khan</a></span> <br>
           Edited on <time class="entry-date" datetime="2066-05-04" pubdate>May 04 2066</time>
