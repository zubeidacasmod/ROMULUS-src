	...
	<article class="post hentry">
		...
		<header class="entry-header">
			<h1 class="entry-title">
				<a href="#" title="Post Heading" rel="bookmark">Metadata: TemporalRelations - BFO Merged </a>
			</h1>
...
$query = "SELECT `OntologyID`, `OntologyName`, `OntologyAcronym`, `OntologyDescription`,
 `CreationDate`, `LatestModifiedDate`, `Version`, `URI` FROM `Ontology` WHERE `OntologyID` =41";
...
$query2 = "SELECT `LanguageName`, `LanguageAcronym`, `LanguageDescription` FROM `Language` WHERE `LanguageName` = (SELECT `LanguageName` from `Ontology` where `OntologyID` = 41)";
...
$query3 = "SELECT `DLExpressivity`, `NumberOfClasses`, `NumberOfIndividuals`, `NumberOfObjectProperties`,`NumberOfDataProperties`, `NumberOfAxioms`
 FROM `Metrics` WHERE `MetricsID` = (SELECT `MetricsID` from `Ontology` where `OntologyID` = 41)";
...
$query55 = "SELECT `LicenceName`, `LicenceDescription`, `LicenceHomePage` from `Licence` where `LicenceName`
 = (SELECT `LicenceName` from `Ontology` where `OntologyID` = 41)";
...
`PersonEmail` IN (SELECT `PersonEmail` from `OntologyPerson` where `OntologyID` = 41)";
...
`ProjectUsageApplication` from `Project` where `ProjectID` IN (SELECT `ProjectID`from `OntologyProject` where `OntologyID` = 41)";
...
in (SELECT `MediationSetID` FROM `MediationSet` where `OntologyID` = 41)";
...
from `MediationSet` where `OntologyID` = 41";
